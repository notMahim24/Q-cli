...
impl BannerState {
    ...
    pub fn tick(&mut self) {
        self.tick += 1;
        match self.tick {
            0..=50 => self.phase = 0,    // Crescent fades in
            51..=95 => self.phase = 1,   // Title appears
            96..=140 => self.phase = 2,  // Tagline types in
...
pub fn render_banner(frame: &mut Frame, area: Rect, state: &BannerState, theme: &Theme) {
    ...
    let content_height = 8 + 1 + 6 + 1 + 1; // crescent + gap + title + gap + tagline
    ...
    // Phase 0+: Crescent (interpolation)
    {
        ...
        let icon_color = interpolate_color(theme.bg, theme.secondary, opacity);

        let lines: Vec<Line> = CRESCENT_ART
            .iter()
            .map(|line| Line::from(Span::styled(*line, Style::default().fg(icon_color))))
            .collect();
